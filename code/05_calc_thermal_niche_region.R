...
for (i in 1:nrow(species_table)) {
  ...
  # pick a region
  if(region == "goa") {
    combined <- pred_df_goa
    tot_area <- goa_area$tot_km2[1]
  }
  if(region == "bc") {
    combined <- pred_df_bc
    tot_area <- bc_area$tot_km2[1]
  }
  if(region == "wc") {
    combined <- pred_df_wc
    totarea <- wc_area$tot_km2[1]
  }
  # combine these 
  # use_goa = FALSE
...
saveRDS(all_temp, paste0("output/temp_niche_",region,".rds"))
