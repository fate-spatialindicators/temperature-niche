...
df_goa <- read.csv(file=here(paste0("output/","goa_presence","_output.csv")), 
              stringsAsFactors = FALSE)
df_bc <- read.csv(file=here(paste0("output/","bc_presence","_output.csv")), 
                  stringsAsFactors = FALSE)
df_wc <- read.csv(file=here(paste0("output/","wc_presence","_output.csv")), 
                 stringsAsFactors = FALSE)
df_wc$region <- "COW"
df_bc$region <- "BC"
df_goa$region <- "GOA"
...
ggsave(g1, file = "plots/Figure_2_alt_presence.png", height=7, width=7)
